...
public class DataService : IDataService
{
    ...
    public event Action? FeaturedClipsChanged;
    ...
    public void SetFeaturedClips(string clips, TimeSpan? lifetime = null)
    {
        if (lifetime.HasValue)
        {
            memCache.Set("FeaturedClips", clips, lifetime.Value);
        }
        else
        {
            memCache.Set("FeaturedClips", clips);
        }

        persistService.SetString("FeaturedClips", clips);
        FeaturedClipsChanged?.Invoke();
    }

    public (bool gotData, string featuredClips) GetFeaturedClips()
    {
        if (memCache.TryGetValue("FeaturedClips", out string? featuredClips))
        {
            if (featuredClips is not null)
            {
                return (true, featuredClips);
            }
        }

        string? temp = persistService.GetString("FeaturedClips");
        if (temp is not null)
        {
            memCache.Set("FeaturedClips", temp);
            return (true, temp);
        }
        return (false, "");
    }
